/**
* Библиотека GL для работы с webgpu
*/
...
//
class Engine {
  static objects = []; //массив всех объектов Engine
  canvasInfo = {
    canvas: undefined,
    context: undefined,
  ...
  static async createModels(engine) {
    for (var modelName in models) {
      ...
      switch (models[modelName].type) {
				case 'cube':
					...
					break;
				case 'gltf':
					engine.models[modelName] = await Promise.resolve(GLTF.createGLTF(engine, models[modelName]));
					break;
				default:
					//...
			}
  ...
  initBuffers(){
    ...
    new Uint32Array(this.indexBuffer.getMappedRange()).set(this.index);
...
//////////////////////////////////////
    ...
    this.MODELMATRIX = m4.xRotate(this.MODELMATRIX, 0.1);
    this.MODELMATRIX = m4.yRotate(this.MODELMATRIX, 1);
    this.MODELMATRIX = m4.zRotate(this.MODELMATRIX, 1);
    ...
    this.VIEWMATRIX = m4.translate(this.VIEWMATRIX, 0,0,-1.5);
  ...
  draw(time) {
    ...
    this.MODELMATRIX = m4.yRotate(this.MODELMATRIX, 0.02);
    this.MODELMATRIX = m4.zRotate(this.MODELMATRIX, 0.01);
    ...
    device.queue.writeBuffer(this.uniformBuffer, 64, this.PROJMATRIX);
      ...
      renderPass.drawIndexed(this.index.length);
